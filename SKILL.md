...
Check the relevant official CleverTap documentation.
Confirm the campaign context where necessary.
Use the documented syntax exactly.
...
unless the property has been provided or confirmed.
...
Normalize the date representation if necessary.
Determine current date/time.
Compare due date with current date/time.
Calculate difference where supported.
Convert difference into the required unit.
...
Understand original intended behavior.
Identify every variable.
Identify every tag.
Identify every filter.
Identify every operator.
Identify every calculation.
Check each against CleverTap compatibility.
Replace unsupported constructs with supported equivalents where possible.
...
Identify what the syntax was intended to accomplish.
Determine whether CleverTap has an equivalent.
If an equivalent exists, convert it.
If no equivalent exists, explain the limitation.
Do not simply delete unsupported syntax.
...
Check the compatibility reference.
Check the official CleverTap documentation.
Check the CleanSlate/LiqP 0.7.9 evidence where relevant.
If still uncertain, explicitly state the uncertainty.
